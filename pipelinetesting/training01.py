...
def train_model():
    ...
    features = df.drop(columns=['price_sats'])
    ...
    for epoch in range(epochs):
        ...
        socketio.emit('training_progress', {'epoch': epoch + 1, 'loss': history.history['loss'][0], 'val_loss': history.history.get('val_loss', [0])[0]})
    ...
    with open('history.npy', 'wb') as f:
        ...
        np.save(f, history.history.get('val_loss', []))
...
if __name__ == "__main__":
    ...
    server_thread = Thread(target=socketio.run, args=(app,), kwargs={'host': '0.0.0.0', 'port': 5300, 'debug': False})
    server_thread.start()
