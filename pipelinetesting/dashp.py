...
def train_model():
    ...
    features = df.drop(columns=['price_usd'])  # Replace 'price_usd' with your actual target column name
    target = df['price_usd']
...
def make_future_predictions(model, last_sequence, future_steps=50):
    predictions = []
    ...
    for _ in range(future_steps):
        ...
        # Ensure prediction has the same shape as current_sequence
        current_sequence = np.vstack([current_sequence[1:], prediction.reshape(1, -1)])
...
def index():
    ...
    return render_template('index.html', logs=parsed_logs)
...
def serve_layout():
    ...
    if df is not None:
        ...
        last_sequence = features.values[-100:]  # Last 100 time steps for prediction

        # Register the loss function and load the model
        keras.utils.get_custom_objects().update({"mse": MeanSquaredError})
        model = keras.models.load_model('model.h5')
        
        predictions = make_future_predictions(model, last_sequence, future_steps=50)

        # Create plots
        recent_fig = go.Figure()
        recent_fig.add_trace(go.Scatter(x=df.index, y=df['price_usd'], mode='lines', name='Recent Data'))
        recent_fig.add_trace(go.Scatter(x=pd.date_range(df.index[-1], periods=50, freq='H'), y=predictions, mode='lines', name='Forecast'))

        return html.Div([
            html.H1('Interactive Dashboard'),
            html.Div('Displaying recent data and forecast.'),
            dcc.Graph(id='recent-graph', figure=recent_fig),
        ])
    else:
        return html.Div([
            html.H1('Interactive Dashboard'),
            html.Div('Failed to load data.')
        ])
